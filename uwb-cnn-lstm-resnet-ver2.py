...
learning_rate = 1e-7
checkpoint_path_ = '/home/usd.local/arash.abbasi/uwb-multipurpose/code/checkpoints_cnn_lstm_ver_seed_10'
...
n_features = 1
n_seq = 10  # 2
n_steps = int(LENGHT_TO_USE / n_seq)  # 250
...
def reading_files():
    ...
    print('unique ', len(np.unique(channel_type_list)))
...
def identity_block(X, f, filters, stage, block):
    ...
    X = TimeDistributed(Conv1D(filters=F1, kernel_size=1, strides=1, padding='valid', name=conv_name_base + '2a',
    ...
    X = TimeDistributed(BatchNormalization(axis=-1, name=bn_name_base + '2a'))(X)
    X = Activation('relu')(X)

    # Second component of main path (≈3 lines)
    X = TimeDistributed(Conv1D(filters=F2, kernel_size=f, strides=1, padding='same', name=conv_name_base + '2b',
                               kernel_initializer=glorot_uniform(seed=None),
                               kernel_regularizer=regularizers.l1_l2(l1=l_1_norm, l2=l_2_norm),
                               bias_regularizer=regularizers.l2(l_2_norm),
                               activity_regularizer=regularizers.l2(l_2_norm)))(X)
    X = TimeDistributed(BatchNormalization(axis=-1, name=bn_name_base + '2b'))(X)
    X = Activation('relu')(X)

    # Third component of main path (≈2 lines)
    X = TimeDistributed(Conv1D(filters=F3, kernel_size=1, strides=1, padding='valid', name=conv_name_base + '2c',
                               kernel_initializer=glorot_uniform(seed=None),
                               kernel_regularizer=regularizers.l1_l2(l1=l_1_norm, l2=l_2_norm),
                               bias_regularizer=regularizers.l2(l_2_norm),
                               activity_regularizer=regularizers.l2(l_2_norm)))(X)
    X = TimeDistributed(BatchNormalization(axis=-1, name=bn_name_base + '2c'))(X)
...
def convolutional_block(X, f, filters, stage, block, s=2):
    ...
    ##### MAIN PATH #####
    # First component of main path
    X = TimeDistributed(Conv1D(F1, kernel_size=1, strides=s, name=conv_name_base + '2a',
                               kernel_initializer=glorot_uniform(seed=None),
                               kernel_regularizer=regularizers.l1_l2(l1=l_1_norm, l2=l_2_norm),
                               bias_regularizer=regularizers.l2(l_2_norm),
                               activity_regularizer=regularizers.l2(l_2_norm)))(X)
    ...
    X_shortcut = TimeDistributed(
        Conv1D(filters=F3, kernel_size=1, strides=s, padding='valid', name=conv_name_base + '1',
               kernel_initializer=glorot_uniform(seed=None),
               kernel_regularizer=regularizers.l1_l2(l1=l_1_norm, l2=l_2_norm),
               bias_regularizer=regularizers.l2(l_2_norm),
               activity_regularizer=regularizers.l2(l_2_norm)))(X_shortcut)
    X_shortcut = TimeDistributed(BatchNormalization(axis=-1, name=bn_name_base + '1'))(X_shortcut)
...
def ResNet50(classes=16):
    ...
    input_shape = (None, n_steps, 1)  # FOR SOME REASONS THAT I DON'T KNOW, WE DO NOT PUT NUMBE OF SEQUESNCE HERE.

    # input_shape = (n_steps, 1) # number os steps, number of features --> look at it this way,
    # # if you have three axial acceleotor data, each for 200 seconds, then you had (200, 3),
    # # now we have a UWB channel with 1000 samples but only 1 feature
    ...
    # Stage 1
    X = TimeDistributed(Conv1D(64, kernel_size=7, strides=2, name='conv1',
                               kernel_initializer=glorot_uniform(seed=None),
                               kernel_regularizer=regularizers.l1_l2(l1=l_1_norm, l2=l_2_norm),
                               bias_regularizer=regularizers.l2(l_2_norm),
                               activity_regularizer=regularizers.l2(l_2_norm)))(X)
    X = TimeDistributed(BatchNormalization(axis=-1, name='bn_conv1'))(X)
    ...
    ####################################
    X_snr = LSTM(256, activation='relu')(X)
    ...
    ####################################
    X_channel_type = LSTM(256, activation='relu')(X)
    ...
    x_channel_type = Dense(4, activation='softmax', name='channel_type',
                           ...
                           activity_regularizer=regularizers.l2(l_2_norm))(X_channel_type)
    ####################################
    X_toa = LSTM(256, activation='relu')(X)
...
def main():
    ...
    train_x, validation_x, train_snr_y, validation_snr_y, train_toa_y, validation_toa_y, train_channel_type_y, validation_channel_type_y = reading_files()

    train_x = train_x.reshape((train_x.shape[0], n_seq, n_steps, n_features))
    validation_x = validation_x.reshape((validation_x.shape[0], n_seq, n_steps, n_features))
    ...
    #               metrics={'snr': ['accuracy', Precision(), AUC()], 'toa': ['mse', 'mae'],
    #                        'channel_type': ['accuracy', Precision(), AUC()]})
